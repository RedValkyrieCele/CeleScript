...
(async function celeScriptShelterSelector() {
    'use strict';
    ...
    function gimmeSettingsesButtonses() {
        ...
        if (commands.querySelector('.cele-settings-btn')) return; // no dupes

        // make the buttonses
        const btn = document.createElement('button');
        btn.className = 'cele-settings-btn';
        btn.title = 'CeleScript Settings';
        btn.setAttribute('aria-label', 'CeleScript Settings');
        ...
        // draw a Cele in it (yes it's not a fimsh)
    ...
    async function saySettingses() {
        ...
        dialog.innerHTML += `
            ...
            <span id="cele-save-msg" style="margin-left:10px;color:#8f8;font-weight:bold;display:none;">Settings saved!</span>
        `;
        ...
        dialog.querySelector('#cele-stash').onclick = async function() {
            ...
            // Show save message
            const msg = dialog.querySelector('#cele-save-msg');
            msg.style.display = 'inline';
            setTimeout(() => {
                msg.style.display = 'none';
                dialog.remove();
                highlightMatches();
            }, 900); // show it for a lil while then close
        };
    ...
    async function gimmeDonoDeets() {
        ...
        try {
            const response = await aypeeeye.shelterDonors({ adopts: adoptIds.join(',')});
            donoMappo = {};
    ...
    const icu = new MutationObserver(() => {
        ...
        highlightMatches();
        updateHighlightyBits();
        // close it if shelter wipey:
        if (!document.getElementById('sheltercommands')) {
            const dlg = document.getElementById('cele-says');
            if (dlg) dlg.remove();
        }
    });
